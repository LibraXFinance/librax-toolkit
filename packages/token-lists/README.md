...
This repo contains main LibraX token list and tools to validate it.
...
- Add `checksum:newlistname`, `generate:newlistname`, `makelist:newlistname` command to `package.json` analogous to LibraX default and extended list scripts.
- Modify `checksum.ts`, `buildList.ts`, `ci-check.ts`, and `default.test.ts` to handle new list

## How to add new tokens to LibraX (extended) token list
...
- Add new tokens to `src/tokens/librax-extended.json` file
...
```shell script
# Fetch the Top100 Tokens on LibraX v2, and update list.
